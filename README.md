      ...
      dns01:
        webhook:
          config:
            ...
            production: true
            ttl: 600
          groupName: acme.shaikabdulrayyan.com
          solverName: godaddy
EOF
```
...
kind: Certificate
metadata:
  name: wildcard-shaikabdulrayyan-com
spec:
  secretName: wildcard-shaikabdulrayyan-com-tls
  renewBefore: 240h
  dnsNames:
  - '*.shaikabdulrayyan.com'
  issuerRef:
...
spec:
  ...
  - hosts:
    ...
    secretName: wildcard-shaikabdulrayyan-com-tls
  rules:
  - host: demo.shaikabdulrayyan.com
    http:
      paths:
      - path: /
        backend:
...
An shaikabdulrayyan Go test file has been provided in [main_test.go]().
...
```bash
$ TEST_ZONE_NAME=shaikabdulrayyan.com. go test .
```

or the following make command
```bash
make test TEST_ZONE_NAME=shaikabdulrayyan.com.
```

The shaikabdulrayyan file has a number of areas you must fill in and replace with your
...
```bash
docker build -t quay.io/khaleelcloud786/cert-manager-webhook-godaddy .
```
- Tag and push it
```bash
